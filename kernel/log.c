...
static void
install_trans(int dev, int recovering)
{
  int tail;
...
{
  read_head(1);
  install_trans(1, 1); // if committed, copy from log to disk
  log[1].lh.n = 0;
...
commit(int dev)
{
  if (log[dev].lh.n > 0) {
    ...
    install_trans(dev, 0); // Now install writes to home locations
    log[dev].lh.n = 0;
